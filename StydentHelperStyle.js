...
// @version      1.5.9-darkfix
// @description  Улучшенный интерфейс расписания для студентов колледжа (с поддержкой тёмной темы)
// @author       AbrikosV + Angelina Maseevskaya (доработки)
...
(function () {
  ...
  // === 🛑 Защита от дублей ===
  const SCRIPT_ID = 'student-helper-stylett-v1.5.9-darkfix';
...
@media (max-width: 480px) {
  ...
  function applyTheme() {
    ...
    // 🔁 Динамическая смена при изменении системной темы (если не зафиксировано вручную)
    mq.onchange = (e) => {
      if (!localStorage.getItem('shs-theme')) {
        const newTheme = e.matches ? 'dark' : 'light';
        document.body.setAttribute('data-theme', newTheme);
        if (switchEl) switchEl.classList.toggle('checked', newTheme === 'dark');
      }
    };
  ...
  const modules = {
    ...
    settingsMenu: {
      ...
      init() {
        ...
        const themeRow = $('#theme-row', this.menu);
        ...
        gearLink.onclick = e => {
          ...
          if (!wasVisible) {
            // Синхронизируем переключатель при открытии
  ...
  function main() {
    ...
    // 1. Сначала добавляем класс — иначе селекторы .shs-enhanced не сработают
    document.body.classList.add('shs-enhanced');

    // 2. Сначала создаём меню настроек → там появляется #theme-switch
    modules.settingsMenu.init();

    // 3. Теперь безопасно применяем тему — переключатель уже в DOM ✅
    applyTheme();
